...
export function parseCommand(text) {
  ...
  // 溜まっている開発メモを見せる。
  // 「開発メモ」の一語だけでなく、「開発メモを見せて」「開発メモの内容をここで送って」の
  // ような文でも受け取れるようにする（メモID 749・750：文になると答えられなかった）。
  // ※「要望 〜」など“メモを残す”指示は上で先に処理済みなので、ここに来るのは“見たい”意図。
  if (/^(開発メモ|要件|開発)$/.test(flat) || /(開発メモ|要件メモ|要望メモ)/.test(flat)) {
    return { kind: "notes" };
  }
  ...
  // SF記録の仕上げ：「SF記録を仕上げて」「商談の記録して」「取りこぼし」
  if (/(sf|ｓｆ).{0,4}記録.{0,4}(仕上|しあ|して|埋め)/i.test(raw) || /商談.{0,4}記録.{0,4}(仕上|しあ|して)/.test(raw) || /取りこぼし/.test(raw)) {
    return { kind: "sfrecord" };
  }
  // SF未紐づけ確認：「未紐づけ」「紐づけ漏れ」「紐付いてない商談」
  if (/未?紐[づ付].{0,3}(け|漏|もれ|け漏)/.test(raw) || /紐[づ付]い?て?(い?ない|てない)/.test(raw)) {
    return { kind: "unlinked" };
  }
  // 直して：AI社員に開発を指示する。「◯◯を直して」「◯◯を作って」「直して」（溜まった分を今すぐ）
  const FIXV = /(直しておいて|直しといて|直して|なおして|開発して|実装して|作って|つくって|対応して|やっておいて|やっといて)/;
  if (FIXV.test(raw)) {
    let inst = raw.replace(FIXV, "").trim()
      .replace(/[をに：:、,。\s　]+$/, "").replace(/^[「『]|[」』]$/g, "").trim();
    if (/^(これ|それ|あれ|さっきの|よろしく|お願いします?|おねがいします?)?$/.test(inst.replace(/[\s　]/g, ""))) inst = "";
    return { kind: "fix", text: inst };
  }
...
export function helpText() {
  return [
    ...
    "・`SF記録を仕上げて` … 商談後のSF活動記録の取りこぼしをまとめて記録",
    "・`未紐づけ` … SFに紐づいていない商談を確認（毎日18時にも自動通知）",
    "・`〇〇を直して` … 指示したことを、AI社員がすぐ直す（本番反映は関所を通す）",
