...
export async function extractFeatureCTags(transcript, meetingDate) {
  ...
  const sys = [
    ...
    "5. customer_industry: 顧客企業の業界・業種。商談中の発言（「うちは製造業で」「介護施設を運営しており」等）から読み取る。自由記述で簡潔に（例：「製造業」「介護・福祉」「IT・ソフトウェア」「飲食」「建設」「小売」「物流」「人材」「教育」「不動産」「医療」「金融」等）。特定できなければ「不明」。",
    "6. customer_response_status: 商談内での顧客の反応区分。「担当者合意」（相手が導入したい・上申したい旨の発言がある）／「案件化」（検討可能の旨だが担当者合意ほど明確な前向き発言ではない）／「不明」のいずれか。",
  ...
  return {
    ...
    customer_industry: raw.customer_industry || "不明",
...
出力はJSONのみ。前置きやコードブロックは書かないでください。
{"chapters":[{"from":0,"to":12,"phase":"ヒアリング","note":"採用課題と母集団形成の悩みを確認"}]}
...
・全体で4〜7章にまとめる。章が細かくなりすぎないようにする（1章は最低でも全体の1割程度の長さ）。
・同じ phase を連続させない。似た話が続くなら1つの章にまとめる。
・note には、その章で何を話したかを20〜35字で書く。「（20〜35字）」のような文字数の指示を本文に書かない。会社名や個人名は入れない。
