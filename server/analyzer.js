...
export async function chatWithData({ messages, material, model }) {
  ...
  const primary = model || process.env.GEMINI_CHAT_MODEL || process.env.GEMINI_MODEL || "gemini-2.5-flash";
  const standard = process.env.GEMINI_CHAT_MODEL || process.env.GEMINI_MODEL || "gemini-2.5-flash-lite";
  const system =
    CHAT_SYSTEM +
    `\n\n【あなたが参照できる商談データ】\n"""\n${material || "（データなし）"}\n"""`;
  ...
  // 一時的な混雑(503/429/UNAVAILABLE)は数回リトライ。Proが混雑なら標準モデルへ降格。
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (!isTransient(e.message)) {
          // 混雑以外のエラーで、次のモデルがあるなら試す。無ければ即throw。
