...
export function mountOauthServer(app) {
  ...
  app.get("/.well-known/oauth-authorization-server", (req, res) => {
    try {
      ...
      console.error("[oauth] metadata failed", e && e.stack ? e.stack : e);
  ...
  app.get("/.well-known/oauth-protected-resource", (req, res) => {
    try {
      ...
      console.error("[oauth] protected-resource metadata failed", e && e.stack ? e.stack : e);
      res.status(500).json({ error: "server_error" });
    }
  });
  ...
  app.post("/oauth/register", async (req, res) => {
    try {
      ...
      await registerOauthClient({ client_id, client_name: body.client_name || "", redirect_uris }).catch((e) =>
      ...
      console.error("[oauth] register failed", e && e.stack ? e.stack : e);
      res.status(500).json({ error: "server_error" });
    }
  });
  ...
  app.get("/oauth/authorize", async (req, res) => {
    try {
      ...
      // ログイン済みなら承認画面を表示（誰の権限で繋がるか分かるようにする）
      ...
      res.end(`<!DOCTYPE html><html lang="ja"><head><meta charset="UTF-8" />
        ...
        <body style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:var(--bg,#f7f5ef);">
          <div style="background:#fff;border-radius:16px;padding:2rem 2.5rem;max-width:420px;box-shadow:0 10px 24px rgba(0,0,0,0.08);text-align:center;">
            ...
            <p style="font-size:13px;color:#666;margin:0 0 20px;">Claude.aiが <b>${esc(user.username)}</b> の権限でkinbotのデータ（案件・商談の実績）を読み取れるようになります。</p>
      ...
      console.error("[oauth] authorize(GET) failed", e && e.stack ? e.stack : e);
  ...
  app.post("/oauth/authorize", async (req, res) => {
    try {
      ...
      if (!redirect_uri) return res.status(400).send("redirect_uriが必要です");
      const user = getUser(req);
      ...
      console.error("[oauth] authorize(POST) failed", e && e.stack ? e.stack : e);
      res.status(500).send("内部エラーが発生しました");
    }
  });
