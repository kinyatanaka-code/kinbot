...
export async function initDb() {
  ...
  // Zoomの通話録音の要約（録音ごとに1回だけ作る。記録に使ったら log_id を入れる）
  await sq(`
    CREATE TABLE IF NOT EXISTS zoom_rec_summaries (
  ...
  await sq(`ALTER TABLE sf_autolaunch ADD COLUMN IF NOT EXISTS doc_notified_at TIMESTAMPTZ;`);   // 申込書回収(SS06)通知の済み
...
// Zoom録音の要約キャッシュ
export async function getZoomRecSummary(recId) {
  if (!pool || !recId) return null;
...
export async function markZoomRecUsed(recId, logId) {
  if (!pool || !recId || !logId) return;
  try {
...
export async function findUnusedZoomSummary(targetId, sinceIso) {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT * FROM zoom_rec_summaries s WHERE s.target_id = $1 AND s.log_id IS NULL AND coalesce(s.summary,'') <> ''
          AND s.at >= $2::timestamptz
          AND NOT EXISTS (SELECT 1 FROM call_logs l WHERE l.zoom_rec_id = s.rec_id)
        ORDER BY s.at DESC LIMIT 1`, [targetId, sinceIso]);
    return rows[0] || null;
  } catch (e) { console.error("[db] findUnusedZoomSummary", e.message); return null; }
}
    ...
    for (const x0 of items) {
      ...
      await pool.query(
        `INSERT INTO call_targets
           (list_id, lead_id, company, person, phone, email, industry, area, memo, assigned_to, sort_order, stage, status, owner_name, extra)
        ...
        [listId, x.leadId || null, x.company || "", x.person || "", x.phone || "",
         ...
         (x.extra && typeof x.extra === "object" && Object.keys(x.extra).length) ? JSON.stringify(x.extra) : null]);
      n++; sort++;
    }
    return n;
...
export async function setSmartLinkApoAt(slug, iso) {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE smart_links SET apo_at = $2, updated_at = now() WHERE slug = $1 RETURNING *`,
      [slug, iso || null]);
    return rows[0] || null;
  } catch (e) { console.error("[db] setSmartLinkApoAt", e.message); return null; }
}
...
// 申込書回収（SS06）通知の候補：まだ申込書回収通知をしていないもの（獲得者・営業担当つき）。
export async function listApoDocCandidates() {
  if (!pool) return [];
  try {
    const { rows } = await pool.query(
      `SELECT a.slug, a.opp_id, a.company, a.title, s.setter_email, s.setter, s.label, s.current_owner
         FROM sf_autolaunch a
         LEFT JOIN smart_links s ON s.slug = a.slug
        WHERE a.opp_id IS NOT NULL
          AND a.doc_notified_at IS NULL
          AND COALESCE(s.setter_email,'') <> ''
        ORDER BY a.tried_at DESC NULLS LAST
        LIMIT 800`);
    return rows || [];
...
export async function markDocNotified(slug) {
  if (!pool || !slug) return;
  try { await pool.query(`UPDATE sf_autolaunch SET doc_notified_at=now() WHERE slug=$1`, [slug]); } catch {}
}
...
export async function nurtureCountsByMember() {
  ...
  try {
    // 数え方は2つを足し合わせる：
    //  ・【ナーチャリング】リストに入っているもの（移動済み。中身はそのまま数える）
    //  ・まだ移していない、ジャッジ／営業フォローのもの
    const { rows } = await pool.query(
...
export async function ensureNurtureList({ owner, createdBy, name }) {
  ...
  try {
    const { rows } = await pool.query(
      `SELECT * FROM call_lists WHERE kind='nurture' AND lower(owner)=lower($1) LIMIT 1`, [owner]);
    if (rows[0]) return rows[0];
    let nm = String(name || "").trim();
    if (!nm) {
      let who = owner;
      try { const u = await pool.query(`SELECT name FROM users WHERE lower(email)=lower($1)`, [owner]); if (u.rows[0] && u.rows[0].name) who = u.rows[0].name; } catch {}
      nm = `【ナーチャリング】${who}`;
    }
    const { rows: ins } = await pool.query(
      `INSERT INTO call_lists (name, owner, created_by, kind) VALUES ($1,$2,$3,'nurture') RETURNING *`,
      [nm.slice(0, 120), String(owner).toLowerCase(), createdBy || null]);
    return ins[0] || null;
  } catch (e) { console.error("[db] ensureNurtureList", e.message); return null; }
}
// 対象リードを、担当ごとのナーチャリングリストへ移す。dryRun=true なら件数だけ。
export async function moveToNurtureLists({ dryRun = true, createdBy = null } = {}) {
  if (!pool) return { 対象: 0, 移動: 0, byMember: [] };
  try {
    const { rows } = await pool.query(
      `SELECT t.id, lower(COALESCE(NULLIF(btrim(t.assigned_to),''), l.owner)) AS email
         FROM call_targets t
         JOIN call_lists l ON l.id = t.list_id
        WHERE ${NURTURE_WHERE} AND COALESCE(l.kind,'') <> 'nurture'`);
    const byM = new Map();
    ...
    if (dryRun) return { 対象: rows.length, 移動: 0, byMember, dryRun: true };
    let 移動 = 0;
