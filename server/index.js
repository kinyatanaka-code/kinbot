...
// よくある日本の苗字（空白の無いフルネームから苗字を切り出すための辞書）。
const COMMON_SURNAMES = new Set((
  "佐藤,鈴木,高橋,田中,伊藤,渡辺,山本,中村,小林,加藤,吉田,山田,佐々木,山口,松本,井上,木村,清水," +
  "斎藤,斉藤,齋藤,齊藤,山崎,阿部,池田,橋本,山下,石川,中島,前田,藤田,後藤,小川,岡田,長谷川,村上," +
  "近藤,石井,坂本,遠藤,藤井,青木,福田,三浦,西村,藤原,太田,松田,原田,岡本,中野,中川,小野,田村,竹内," +
  "金子,和田,中山,石田,上田,森田,柴田,酒井,工藤,横山,宮崎,宮本,内田,高木,安藤,島田,谷口,大野," +
  "高田,丸山,今井,河野,藤本,村田,武田,上野,杉山,増田,小島,平野,大塚,千葉,久保,松井,岩崎,木下,野口," +
  "松尾,菊地,菊池,野村,渡部,菅原,小山,大西,山内,平田,吉川,中田,武藤,新井,小松,田口,山中,中西," +
  "服部,樋口,福島,川口,大久保,松岡,菅野,成田,小池,岩田,水野,吉村,秋山,若林,竹田,小沢,小澤,栗原," +
  "大島,浜田,森本,松浦,山根,大石,吉岡,黒田,金城,岡崎,川崎,飯田,古川,今村,白石,内藤,松下,平井,永井," +
  "西田,松村,福井,川上,前川,本田,榎本,浅野,星野,松原,土屋,川村,神谷,西川,広瀬,大森,望月,永田,三宅," +
  "中尾,尾崎,牧野,岸本,西山,片山,浦林,植野,森川,江田,受川,松山,笹原,楢原,黒柳,真野,杉本,兒玉,岩元," +
  "樽井,飯島,中澤,森川,森田,林田,原田,関口,堀内,南田"
).split(",").map((s) => s.trim()).filter((s) => s && s.length >= 2 && s.length <= 4 && /^[\u4e00-\u9fff]+$/.test(s)));

// 担当者名から苗字を取り出す。空白があればその前。無ければ辞書でいちばん長く一致する苗字を採用。
function surnameOf(person) {
  const p = String(person || "").trim();
  if (!p) return "";
  const bySpace = p.split(/[\s　]+/)[0];
  if (bySpace !== p) return bySpace;   // 「山田 太郎」→「山田」
  for (let n = Math.min(4, p.length); n >= 1; n--) {
    if (COMMON_SURNAMES.has(p.slice(0, n))) return p.slice(0, n);   // 「高橋秀斗」→「高橋」
  }
  return p.length >= 2 ? p.slice(0, 2) : p;   // 辞書に無ければ2文字を目安に
}
...
app.post("/api/calls/from-csv", async (req, res) => {
  try {
    ...
    for (const r of 行) {
      ...
      // 担当者名は苗字だけを使う。空白があればその前、無ければよくある苗字の辞書で切り出す。
      const 姓 = surnameOf(person);
...
app.post("/api/calls/assign", async (req, res) => {
  try {
    ...
    const r = await assignCallTargets(listId, who, { onlyUnassigned: !全部やり直す });
    const n = r.assigned || 0;
    console.log(`[kincall] ${n}件を${who.length}人に配りました（ジャッジ除外 ${r.judgeKept || 0}）by ${req.user}`);
    res.json({ ok: true, 配った数: n, 人数: who.length, ジャッジ除外: r.judgeKept || 0 });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.get("/api/calls/apo-dashboard", async (req, res) => {
  try {
    ...
    try {
      ...
      for (const p of insideP) {
        const i = 金額順.indexOf(p.インセンティブ || 0);
        if (i >= 0) {
          p.順位 = i + 1;
          p.タイ = insideP.filter((q) => (q.インセンティブ || 0) === (p.インセンティブ || 0)).length > 1;
        }
      }
...
app.get("/api/calls/apo-goals-cells", async (req, res) => {
  try {
    ...
    const keys = String(req.query.keys || "").split(",").map((x) => x.trim()).filter(Boolean).slice(0, 100);
    if (!keys.length) return res.json({ ok: true, goals: {} });
    res.json({ ok: true, goals: await getApoGoalsByKeys(period, keys) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// アポ目標の保存（手入力）。subject＝group/sales/inside またはメンバーのメール、
// period＝day/week/month、periodKey＝その期間のキー、metric＝コール/接触/アポ。
// 入れた期間にそのまま保存する（他の期間へ配分・自動入力はしない）。
...
app.get("/api/calls/process", async (req, res) => {
  try {
    ...
    // 記録あり商談（実施の突合用）：初回タイトルの録音あり商談を 会社名|商談日 の集合にする。
    const buildDone = async (from, to) => {
    ...
    // 記録あり商談を 会社名|商談日 → 実施した人(owner) のMapにする（担当補完・実施専任の集計に使う）。
    const buildDoneOwner = async (from, to) => {
      const meetings = await listMeetings({ isAdmin: true, from, to, limit: 2000, light: true }).catch(() => []);
      const map = new Map();
      for (const m of meetings) {
        if (!対象タイトル(m.title || "")) continue;
    ...
    // 1区切りぶんの「全体合計 設定/実施」。設定＝会社×商談日でユニークな初回商談、実施＝そのうち記録があるもの（内数）。
    ...
    const done = await buildDone(from, to);            // 会社|商談日 の集合（記録あり）
    const doneOwner = await buildDoneOwner(from, to);  // 会社|商談日 -> 実施した人(owner)
    const apos = await aposByMeetingDate(from, to).catch(() => []);

    // 8月だけ「実施専任」（設定に数えない・実施は記録ベース）＝中澤良太。期間が8月にかかるときのみ。
    const 実施専任 = new Set();
    if (String(month) === "2026-08") {
      for (const [em, nm] of closers) if (/中澤良太/.test(String(nm))) 実施専任.add(em);
    }

    // 設定：会社名×商談日でユニーク化し、担当を1つ決める。
    // 決め方：current_owner（クローザー・実施専任は除く）→ 記録owner → 招待主催者 → その他/未定。
    ...
    for (const a of apos) {
      ...
      const cur = uniqSet.get(key);
      if (cur === undefined) uniqSet.set(key, cand);
      else if (!cur && cand) uniqSet.set(key, cand);   // 空→クローザー候補で上書き
    }
    ...
    for (const [key, owner] of uniqSet) {
      if (owner) continue;
      let em = doneOwner.get(key) || "";                                   // 記録の担当（実施済みなら確実）
      if (!closers.has(em) || 実施専任.has(em)) em = "";
      if (!em) { const io = inviteOwner.get(key); if (closers.has(io) && !実施専任.has(io)) em = io; } // 招待主催者
      uniqSet.set(key, em);   // それでも空なら「その他/未定」
    }

    const stat = new Map();
    const ensure = (b) => { if (!stat.has(b)) stat.set(b, { 設定: 0, 実施: 0 }); return stat.get(b); };
    for (const [key, owner] of uniqSet) {
      const bucket = closers.has(owner) ? owner : "__other__";
      const s = ensure(bucket); s.設定 += 1;
      if (done.has(key)) s.実施 += 1;   // 設定の内数（記録あり）
    }
    // 実施専任（中澤）は設定に数えず、実施＝記録の担当が本人の件数
    for (const em of 実施専任) {
      let n = 0; for (const owner of doneOwner.values()) if (owner === em) n++;
      const s = ensure(em); s.設定 = 0; s.実施 = n;
    }
...
async function notifyDevNoteDone(note) {
  try {
    ...
    // みんなが見る通常のチャットにも、開発が完了したことを流す
    await notifyChat(
      `🛠️ 開発しました（${kindLabel}対応 完了）\n・${note.title || ""}${who ? `\n（要望：${who}）` : ""}`
    ).catch(() => {});
...
app.post("/api/ai/chat", async (req, res) => {
  try {
    ...
    // 頭脳（LLM）の選択：gemini / claude(=anthropic) / groq
    const pmap = { claude: "anthropic", anthropic: "anthropic", gemini: "gemini", groq: "groq" };
    const provider = pmap[String(req.body?.provider || "gemini").toLowerCase()] || "gemini";
    const providerLabel = provider === "anthropic" ? "claude" : provider;   // 画面に返す表示名
    ...
    try {
      ...
      return res.json({ ok: true, reply: `いま考えがまとまりませんでした（${e.message}）。もう一度お願いできますか。`, provider: providerLabel });
    }
    ...
    res.json({ ok: true, reply, provider: providerLabel, action });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// キツツキ（CEO）と会話する ここまで

// 頭脳（LLM）の接続状況：どのプロバイダのAPIキーが入っているか（Railwayの環境変数を確認できる）。
app.get("/api/ai/llm-status", async (req, res) => {
  try {
    if (!isAiOwner(req)) return res.status(403).json({ error: "権限がありません" });
    res.json({
      ok: true,
      default: (process.env.LLM_PROVIDER || "gemini").toLowerCase(),
      providers: {
...
// 頭脳（LLM）の接続テスト：選んだプロバイダに軽く1回だけ投げて、つながるか確かめる。
// フォールバックは使わない（そのプロバイダ自身の疎通を見るため）。
app.post("/api/ai/llm-test", async (req, res) => {
  try {
    if (!isAiOwner(req)) return res.status(403).json({ error: "権限がありません" });
    const pmap = { claude: "anthropic", anthropic: "anthropic", gemini: "gemini", groq: "groq", openai: "openai" };
    const provider = pmap[String(req.body?.provider || "").toLowerCase()] || (process.env.LLM_PROVIDER || "gemini").toLowerCase();
    const label = provider === "anthropic" ? "claude" : provider;
    const t0 = Date.now();
    try {
      const reply = String(await callLLMPublic(
        "あなたは接続テストの応答役です。", "『OK』とだけ短く返してください。", 20,
        { provider, json: false, fallback: false }) || "").trim();
      res.json({ ok: true, provider: label, ms: Date.now() - t0, reply: reply.slice(0, 80) });
    } catch (e) {
      res.json({ ok: false, provider: label, error: e.message });
    }
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
app.post("/api/meetings/:id/extract", async (req, res) => {
  try {
    // モデル指定（claude/gemini/groq）。未指定なら設定の既定に従う。
    let p = String((req.body && req.body.provider) || "").toLowerCase();
    if (!["anthropic", "gemini", "groq"].includes(p)) p = "";
...
app.get("/api/judge-provider", async (req, res) => {
  try {
    const s = await getSettings();
    const v = s && ["anthropic", "gemini", "groq"].includes(s.judgeProvider) ? s.judgeProvider : "";
    res.json({ provider: v });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
const BUILD_TAG = "2026-09-04ek 長い商談で要約が途中で切れる（MAX_TOKENS）件を改善：文字起こしが長いときは前半・後半に分けて要点を作り、それをもとに要約を生成するようにし、出力の上限も広げた。あわせて、Groqのモデルに文字起こし専用（whisper等）が選ばれていても要約・分析には使わないようにした。前回(ej)：文字起こしのモデル修正。";
...
async function loadSkipInviters() {
  ...
  const fromSettings = raw.split(/[,、\n]/).map((x) => x.trim()).filter(Boolean);
  _skipInviters = [...new Set([...fromSettings, ...MANDATORY_SKIP])];
