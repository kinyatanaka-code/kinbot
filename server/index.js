...
const BUILD_TAG = "2026-09-02ac ホーム改善：ツールのアイコンを検索ボックスのすぐ右横（上部）へ移動（アイコンのみ・名前はツールチップ）。今日の商談の「SF確認」ボタンを削除（AI社員が監査するため不要）。自分のアポ一覧から担当者を変更できるように（担当セレクト＋即時反映）。前回：定例通知の共通化";
