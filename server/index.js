...
import { analyzerInfo, analyzeMeeting, analyzeDeep, freeAnalyze, chatWithData, enrichCompany, generateThanks, THANKS_PROMPT, getCheckItems, classifyMeetingKind, extractFirstMeeting, extractReMeeting, extractFeatureCTags, enrichCompanyAttributes, generateFeatureCInsights } from "./analyzer.js";
import {
  googleConfigured,
  authUrl,
  exchangeCode,
...
async function runExtraction(botId) {
  ...
  if (kind === "初回商談") {
    ...
    // (依頼書4.3の「Feature Aのコア指標の安定運用を優先」に沿う)
    if (deal) {
      (async () => {
        try {
          const tags = await extractFeatureCTags(transcript, meetingDateStr);
          // customer_response_statusの後処理（依頼書4.2）：
          // Feature Aで即失注判定なら"失注"に上書きする。deal.statusを直接見るのが単純で確実。
          ...
          // 集計除外セーフティネット：tag_confidenceがlowならhiring_type_needは"不明"扱いにする
          // （依頼書4.2で「集計から自動除外する」と明記）。DB上は保持し、集計クエリ側で判断可能に。
          await upsertDealFeatureTags(deal.deal_id, {
            first_meeting_date: meetingDateStr,
            owner: repName || owner,
            team,
            ...
            customer_industry: null,      // フェーズ3のエンリッチメントで埋める
            target_job_type: null,        // フェーズ3のエンリッチメントで埋める
