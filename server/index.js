...
const BUILD_TAG = "2026-09-04en 文字起こしが『File is not in an ACTIVE state』で失敗する件を修正。動画をGeminiにアップロードしたあと、使える状態になるまで確実に待つようにし（最大3分・状態をログ出力）、それでも準備中のエラーが返った場合は10秒待って再試行するようにした。前回(em)：文字起こしの作り直し。";
