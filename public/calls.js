...
(function injectKcComboStyle() {
  ...
  s.textContent = `
    ...
    .kc-slot-item{display:block;width:100%;text-align:left;border:1.5px solid #cfe0d9;background:#ffffff;border-radius:10px;padding:11px 13px;margin-bottom:9px;cursor:pointer;transition:all .12s;}
  ...
  const m = openModal(相手名 || (histOnly ? "これまでのやり取り" : "記録する"), `
    <div class="kc-two${histOnly ? " kc-two-histonly" : ""}">
      ...
      <div class="kc-two-r">
        ...
        <div class="kc-zsum" id="kcZsum" hidden></div>
    ...
    try {
      const r = await fetch(`/api/calls/targets/${encodeURIComponent(id)}/record`, {
        ...
        body: JSON.stringify({
          ...
          absentRank,
          memo: m.el.querySelector("#kcMemo").value,
          // 説明に入れたZoom録音の要約（使用済みにして、後から二重に入らないようにする）
          zoomRecId: (m._zoomRecId && /【通話録音の要約】/.test(m.el.querySelector("#kcMemo").value)) ? m._zoomRecId : "",
          status: m.el.querySelector("#kcStatus").value,
...
//  結果（担当者不在など）は架電者が選ぶ。
function wireZoomSummary(m, id) {
  if (!_zoomOn) return;
  const box = m.el.querySelector("#kcZsum");
  const memo = m.el.querySelector("#kcMemo");
  if (!box || !memo) return;
  const MARK = "【通話録音の要約】";
  ...
  const watch = async () => {
    const my = ++watching;
    show('<span class="kc-zsum-wait">📞 通話が終わると、Zoomの録音から要約をここに入れます…</span>');
    for (let i = 0; i < 32; i++) {   // 15秒ごと × 32回 ≒ 8分
      await new Promise((r) => setTimeout(r, 15000));
      if (my !== watching || !document.body.contains(m.el) || memo.value.includes(MARK)) return;
      ...
      <div class="kc-admin">
        ${iAmCloser ? `<div class="kc-adcard">
          <details class="rr-details">
            ...
            <div style="margin-top:12px;padding-top:10px;border-top:1px solid #eef3f0">
              <div class="note">リサイクルのリードを<b>インサイド全員</b>に配ります。取れそうな順（温度A→B→C）で、各リードは<b>元のグループのその人の復活リスト</b>へ入ります（現在使われていない・アポ獲得は配りません）。まず試算で確認してください。</div>
              <div style="display:flex;gap:8px;align-items:center;margin-top:6px;flex-wrap:wrap">
                <label style="font-size:12px">1人あたり <input type="number" id="rdPer" value="50" min="1" max="500" style="width:64px;padding:4px 6px;border:1px solid #d7e0db;border-radius:8px;text-align:center" /> 件</label>
                <button type="button" class="btn ghost" id="rdDry">試算する</button>
                <button type="button" class="btn" id="rdRun">割り振る（実行）</button>
...
async function loadRecycleRules() {
  ...
  // リサイクルの割り振り（試算・実行）
  const rdDry = $("rdDry"), rdRun = $("rdRun");
  if (rdDry && !rdDry.dataset.wired) {
    ...
    rdRun.addEventListener("click", async () => {
      if (!confirm(`リサイクルのリードを、インサイド全員に1人 ${per()}件ずつ配ります。\n各リードは元のグループのその人の復活リストへ移り、担当もその人になります。\nよろしいですか？`)) return;
      rdRun.disabled = true; say("rdSt", "配っています…");
      try {
        const d = await (await fetch("/api/calls/recycle-distribute", {
          method: "POST", headers: { "content-type": "application/json" },
...
function nmRenderCards() {
  ...
  for (const x of _nmLists) { const k = String(x.owner || "").toLowerCase(); if (!k) { if (!["アーカイブ", "リサイクル"].includes(String(x.name || "").trim())) orphan.push(x); continue; } if (!byOwner.has(k)) byOwner.set(k, []); byOwner.get(k).push(x); }
  ...
  const totalNur = _nmLists.reduce((s, x) => s + Number(x.ナーチャリング || 0), 0);
...
async function nmClImportMissing() {
  ...
  try {
    ...
    const lines = (p.plan || []).slice(0, 12).map((x) => `・${x.name}：${x.count}社`).join("\n");
    if (!confirm(`kincallのどのリストにも無いクロス失注の会社が ${p.total} 社あります。\n1つのリスト「クロス失注（未割り当て）」にまとめて取り込み、失注リストに入れます（担当は付けません）。\n\n商談所有者の内訳：\n${lines}\n\n担当者名は主.取引先責任者、電話・メールは取引先責任者（無ければ取引先）から入れます。\n取り込みますか？`)) { if (st) st.textContent = ""; return; }
    if (st) st.textContent = "取り込んでいます…";
    const d = await call(false);
    await nmLoad();
    const st2 = $("nmClConSt"); if (st2) st2.textContent = `${d.imported || 0}社を「クロス失注（未割り当て）」に取り込みました`;
  } catch (e) { if (st) st.textContent = "失敗：" + (e.message || ""); }
...
function nmRenderDetail() {
  ...
  body.querySelectorAll(".nm-selchk").forEach((c) => c.addEventListener("change", () => {
    ...
    const row = c.closest(".nm-lcard"); if (row) row.classList.toggle("sel", c.checked);
    nmUpdateEditBar();
  }));
...
async function nmDelete(listId, name) {
  ...
  const st = $("nmOpSt"); if (st) st.textContent = "削除しています…";
  try {
    const r = await fetch(`/api/calls/lists/${encodeURIComponent(listId)}`, { method: "DELETE" });
    const d = await r.json().catch(() => ({}));
    if (!r.ok || !d.ok) throw new Error(d.error || "");
    await nmFetch(); nmRenderRoot();
...
async function edImportCsv(ev) {
  ...
  const cCo = findCol(["会社名", "企業名", "取引先名", "会社", "企業", "社名"]);
  const cEmp = findCol(["従業員数", "従業員", "社員数", "社員"]);
  const cHire = findCol(["採用人数", "採用予定人数", "採用数"]);
  const cMed = findCol(["媒体掲載", "媒体"]);
  // 担当者名（相手先の人）：「担当者名」「担当者」「主.取引先責任者」など。メール・電話の列は除く。
  const cPer = (() => {
    const bad = (h) => /メール|mail|電話|phone|tel|役職|部署/i.test(h);
    for (const w of ["担当者名", "主取引先責任者", "取引先責任者名", "取引先責任者", "担当者"]) {
      const nw = norm(w).replace(/[.．:：]/g, "");
      const i = H.findIndex((h) => h.replace(/[.．:：]/g, "") === nw && !bad(h));
      if (i >= 0) return i;
    }
    for (const w of ["主取引先責任者", "取引先責任者"]) {
      const nw = norm(w);
      const i = H.findIndex((h) => h.replace(/[.．:：]/g, "").includes(nw) && !bad(h));
      if (i >= 0) return i;
    }
    return -1;
  })();
  ...
  if (cId >= 0) {
    ...
    rows = grid.slice(1).map((r) => ({
      ...
      media_tags: cMed >= 0 ? r[cMed] : undefined, person: cPer >= 0 ? r[cPer] : undefined, group: cGrp >= 0 ? r[cGrp] : "", assigned_name: cAsg >= 0 ? r[cAsg] : "", assigned_email: cAsgE >= 0 ? r[cAsgE] : "",
    })).filter((r) => String(r.id || "").trim());
  } else if (cCo >= 0) {
    // リードIDが無いCSV（会社名＋従業員数など）：いま表示中の編集テーブルの行に会社名で突合する
    byCompany = true;
    if (cEmp < 0 && cHire < 0 && cMed < 0 && cPer < 0) { alert(`CSVに「従業員数」などの列（従業員数・採用人数・媒体掲載・担当者名／主.取引先責任者）が見つかりませんでした。\n見出しを「従業員数」にしてください。\n\n今のCSVの見出し：\n${head.join(" / ")}`); return; }
    const byCo = new Map();
    ...
    for (const gr of grid.slice(1)) {
      ...
      coMatched++;
      const emp = cEmp >= 0 ? gr[cEmp] : undefined, hire = cHire >= 0 ? gr[cHire] : undefined, med = cMed >= 0 ? gr[cMed] : undefined, per = cPer >= 0 ? gr[cPer] : undefined;
      for (const er of ers) map.set(String(er.id), { id: er.id, employees: emp, hires: hire, media_tags: med, person: per });   // その会社の全リードに反映
    }
    rows = [...map.values()];
  ...
  try {
    ...
    const msg = (byCompany
      ...
      : `${rows.length}件を反映します。\n担当変更 ${s.assignedChanged || 0}／会社名 ${s.companyChanged || 0}／担当者名 ${s.personChanged || 0}／従業員数など ${s.fieldChanged || 0}／グループ ${s.groupChanged || 0}`) +
...
function edRenderBody() {
  ...
  tb.querySelectorAll(".ed-group").forEach((sel) => sel.addEventListener("change", async () => {
    ...
    try {
      const r = await fetch(`/api/calls/lists/${encodeURIComponent(list)}/group`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ groupId: gid || null }) });
      if (!r.ok) throw new Error();
      _edRows.forEach((x) => { if (String(x._listId) === String(list)) { x._groupId = gid; x._group = gname; } });
      if (_edLists[list]) { _edLists[list].group_id = gid; _edLists[list].group_name = gname; }
  ...
  tb.querySelectorAll(".ed-owner").forEach((sel) => sel.addEventListener("change", async () => {
    ...
    try {
      ...
      _edRows.forEach((x) => { if (String(x._listId) === String(list)) x._owner = owner; });
      if (_edLists[list]) _edLists[list].owner = owner;
      sel.style.outline = "2px solid #1d9e75"; setTimeout(() => (sel.style.outline = ""), 800);
